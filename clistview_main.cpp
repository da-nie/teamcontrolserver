...
BEGIN_MESSAGE_MAP(CListView_Main,CView)
 ON_WM_SIZE()
 ON_WM_PAINT()//без ON_WM_PAINT список вообще не отрисовываетс€!!! ’от€ должен бы.
 ON_WM_RBUTTONDOWN()
...
{ 
 ...
 cListCtrl.InsertColumn(0,"ѕользователь",LVCFMT_CENTER,width/5);
 cListCtrl.InsertColumn(1,"ƒолжность",LVCFMT_CENTER,width/5);
 cListCtrl.InsertColumn(2,"Ћогин",LVCFMT_CENTER,width/5);
 cListCtrl.InsertColumn(3,"ѕароль",LVCFMT_CENTER,width/5);
 cListCtrl.InsertColumn(4,"јккаунт",LVCFMT_CENTER,width/5);
...
{
 //получаем выбранного пользовател€
 SUser sUser;
...
afx_msg void CListView_Main::OnCommand_Menu_List_DeleteUser(void)
{
 ...
 SUser sUser;
 if (GetSelectedUser(sUser)==false) return;//ошибка
 CString name="”далить пользовател€ ";
 name+=sUser.Name;
 name+=" ?";
 if (MessageBox(name,"ѕодтверждение",MB_YESNO|MB_DEFBUTTON2)!=IDYES) return;
 CListCtrl &cListCtrl=GetListCtrl();
 if (cListCtrl.GetSelectedCount()==0) return;//ничего не выбрано
 long selected_item=cListCtrl.GetSelectionMark();
 CDocument_Main *cDocument_Main_Ptr=GetDocument();
 cDocument_Main_Ptr->DeleteUser(selected_item); 
 UpdateList();
}
//----------------------------------------------------------------------------------------------------
...
afx_msg void CListView_Main::OnCommand_Menu_List_SetLeader(void)
{
 ...
 if (GetSelectedUser(sUser)==false) return;//ошибка
 sUser.Leader=true;
 //записываем выбранного пользовател€
...
void CListView_Main::UpdateList(void)
{
 ...
 while(iterator!=iterator_end)
 {
  ...
  cListCtrl.SetItem(&item);
  item.iSubItem=4;
  if (sUser.Leader==true) item.pszText="–уководитель";
                     else item.pszText="—отрудник";
  cListCtrl.SetItem(&item);
  index++;
  iterator++;
 }
...
//----------------------------------------------------------------------------------------------------
//получить выбранного пользовател€ из класса документа
//----------------------------------------------------------------------------------------------------
bool CListView_Main::GetSelectedUser(SUser &sUser)
{
 ...
 long selected_item=cListCtrl.GetSelectionMark();
 CDocument_Main *cDocument_Main_Ptr=GetDocument();
 return(cDocument_Main_Ptr->GetUser(selected_item,sUser));
}
//----------------------------------------------------------------------------------------------------
//записать выбранного пользовател€ в класс документа
//----------------------------------------------------------------------------------------------------
void CListView_Main::SetSelectedUser(const SUser &sUser)
{
 ...
 long selected_item=cListCtrl.GetSelectionMark();
 CDocument_Main *cDocument_Main_Ptr=GetDocument();
 cDocument_Main_Ptr->ChangeUser(selected_item,sUser);
 //обновл€ем список пользователей
 UpdateList();
}
